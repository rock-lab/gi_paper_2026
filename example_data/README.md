...
Most files are regenerated by `../make_example_data.py`, which row-filters the real
...
`gi_input/`, `counts/`, `expected/`, and `guide_name_map.tsv`.

The two `experiment_metadata_exp{1,2}.csv` files are **curated separately** (they
are *not* produced by `make_example_data.py`): each maps its screen's timepoints
to generations, ATc condition, and the sliced count-file path under `counts/`.
